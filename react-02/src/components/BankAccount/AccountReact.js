...
class AccountCTRL extends Component {
  ...
  deposit = (amt, key1) => {
    // if/else
    const changingAccount = this.state.x.accountsHolder.find(
      (x) => x.key === key1
    );
    const changingAccountCards = this.state.x.accountCards.find(
      (x) => x.key === key1
    );
  ...
  withdraw = (amt, key1) => {
    // if/else
    const changingAccount1 = this.state.x.accountsHolder.find(
      (x) => x.key === key1
    );
    const changingAccountCards1 = this.state.x.accountCards.find(
      (x) => x.key === key1
    );
    // (this.state.x.accountCards.find(x => x.key === key1)).balance=changingAccount.balance
  ...
  delete = (key1) => {
    const deletedAccount = this.state.x.accountsHolder.find(
      (x) => x.key === key1
    );
  ...
  render() {
    return (
      <div
        className="account-details container text-left"
        onClick={this.clickMe}
      >
        <div className="row">
          <div className="col">
            <p>
              <strong>Total Accounts Value:</strong>${this.state.totalArray[0]}
            </p>

            <p>
              <strong>Total Number of Accounts:</strong>
              {this.state.totalArray[1]}
            </p>

            <p>
              <strong>Smallest Account Name:</strong>
              {this.state.totalArray[2]} <strong>Value:</strong>$
              {this.state.totalArray[3]}
            </p>

            <p>
              <strong>Largest Account Name:</strong>
              {this.state.totalArray[4]} <strong>Value:</strong>$
              {this.state.totalArray[5]}
            </p>
          </div>
          ...
          <div className="col">
            ...
            <div>
              ...
              <input type="text" id="idaccountN" required />
            </div>

            <div>
              <label>Starting Balance: </label>
              <input type="number" id="idstartingB" required />
            </div>
